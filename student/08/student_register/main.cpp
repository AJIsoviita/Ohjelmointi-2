    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            else
            {
                ...
                for(auto pair: student_numbers)
                {
                    if (pair.first == parts[1])
                    {
                        ...
                        if (number.find_first_not_of("0123456789 ") == string::npos)
                            pair.second->phone_number = number;
                        else
                        {
                ...
                if (!check)
                {
                    cout << "There is no sutdent with the given number!" << endl;
                    cout << endl;
                }
                else
                {
                    ...
                    for(auto pair : user_ids)
                        output << pair.second->student_number << ";" << pair.second->user_id << ";" << pair.second->name << ";" << pair.second->phone_number << ";" << pair.second->email << ";" << pair.second->skype << "\n";
                     output.close();
                }
            }
