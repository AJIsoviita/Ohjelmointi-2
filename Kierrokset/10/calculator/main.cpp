...
int main() {
    ...
    while ( true )
    {
                ...
                command == "INCREASE")
        {
            if(pieces.size() == 3)
            {
                if(string_to_double(pieces.at(1),test) and
                        string_to_double(pieces.at(2), test))
                {
                    double result = addition(stoi(pieces.at(1)), stoi(pieces.at(2)));
                    cout << result << endl;
                }
                else
                ...
                command == "MINUS" or
                command == "SUBTRACTION" or
                command == "DIFFERENCE" or
                command == "SUBTRACT" or
                command == "DECREASE")
        {
            if(pieces.size() == 3)
            {
                if(string_to_double(pieces.at(1),test) and
                        string_to_double(pieces.at(2), test))
                {
                    double result = subtraction(stoi(pieces.at(1)), stoi(pieces.at(2)));
                    cout << result << endl;
                }
                else
                    cout << "Error: a non-number operand." << endl;
                ...
                command == "MULTIPLY")
        {
            if(pieces.size() == 3)
            {
                if(string_to_double(pieces.at(1),test) and
                        string_to_double(pieces.at(2), test))
                {
                    double result = multiplication(stoi(pieces.at(1)), stoi(pieces.at(2)));
                    cout << result << endl;
                }
                else
                    cout << "Error: a non-number operand." << endl;
                ...
                command == "DIVIDE")
        {
            if(pieces.size() == 3)
            {
                if(string_to_double(pieces.at(1),test) and
                        string_to_double(pieces.at(2), test))
                {
                    double result = division(stoi(pieces.at(1)), stoi(pieces.at(2)));
                    cout << result << endl;
                }
                else
                    cout << "Error: a non-number operand." << endl;
                ...
                command == "EXP")
        {
            if(pieces.size() == 3)
            {
                if(string_to_double(pieces.at(1),test) and
                        string_to_double(pieces.at(2), test))
                {
                    double result = exponent(stoi(pieces.at(1)), stoi(pieces.at(2)));
                    cout << result << endl;
                }
                else
                    cout << "Error: a non-number operand." << endl;
